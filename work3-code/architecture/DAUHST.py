    ...
    def forward(self, x):
        ...
        if self.only_local_branch:
            ...
            q1, k1, v1 = map(lambda t: rearrange(t, 'b (h b0) (w b1) c -> b (h w) (b0 b1) c',
                                              b0=w_size[0], b1=w_size[1]), (q1, k1, v1))# (b,hw/mm,mm,c)
            q1, k1, v1 = map(lambda t: rearrange(t, 'b n mm (h d) -> b n h mm d', h=self.heads//2), (q1, k1, v1))#(b,hw/mm,ha,mm,d)
            q1 *= self.scale
            sim1 = einsum('b n h i d, b n h j d -> b n h i j', q1, k1)#(b,hw/mm,ha,mm,mm)
...
class HST(nn.Module):
    def __init__(self, in_dim=29, out_dim=28, dim=28, num_blocks=[1,1,1]):
...
class DAUHST(nn.Module):
    ...
    def __init__(self, num_iterations=1):
        ...
        self.para_estimator = HyPaNet(in_nc=24, out_nc=num_iterations*2)
        self.fution = nn.Conv2d(24, 24, 1, padding=0, bias=True)
        self.num_iterations = num_iterations
        self.denoisers = nn.ModuleList([])
        for _ in range(num_iterations):
            self.denoisers.append(
                HST(in_dim=25, out_dim=24, dim=24, num_blocks=[1,1,1]),
            )
    def initial(self, y, Phi):
        ...
        y = y / nC * 2
        bs,row,col = y.shape
        y_shift = torch.zeros(bs, nC, row, row).cuda().float()
        for i in range(nC):
            y_shift[:, i, :, :] = y[:, :, step * i:step * i + col - (nC - 1) * step]
        z = self.fution(y_shift)
