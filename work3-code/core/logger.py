...
def parse(args, isTrainCASSI=False):
    if isTrainCASSI:
        phase = args.phase
        ...
        opt['phase'] = phase
        opt['datasets']['train']['r_resolution'] = args.patch_size
        opt['datasets']['train']['batch_size'] = args.batch_size
        opt['datasets']['val']['r_resolution'] = 256
        opt['datasets']['val']['batch_size'] = 10
        opt['model']['diffusion']['image_size'] = 256
        opt['datasets']['train']['data_len'] = args.epoch_sam_num
    else:
        phase = args.phase
        opt_path = args.config
        gpu_ids = args.gpu_id
        ...
        experiments_root = os.path.join(
            'experiments', '{}_{}_{}'.format(opt['name'], opt['model']['which_model_G'], get_timestamp()))
