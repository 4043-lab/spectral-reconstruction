...
def generate_random_masks(mask_path, batch_size):
...
def gen_meas_torch_my(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    temp = shift(mask3d_batch * data_batch, 2)
    meas = torch.sum(temp, 1)
...
def init_mask(mask_path, mask_type, batch_size):
    ...
    if mask_type == 'Phi':
        ...
        input_mask = generate_random_masks(mask_path, batch_size)
    elif mask_type == None:
        input_mask = None
...
def init_meas_my(gt, mask, input_setting):
    input_meas = None
    if input_setting == 'H':
        input_meas = gen_meas_torch_my(gt, mask, Y2H=True, mul_mask=False)
    elif input_setting == 'HM':
        input_meas = gen_meas_torch_my(gt, mask, Y2H=True, mul_mask=True)
    elif input_setting == 'Y':
        input_meas = gen_meas_torch_my(gt, mask, Y2H=False, mul_mask=False)
    elif input_setting == 'X':
        input_meas = gt
    return input_meas
