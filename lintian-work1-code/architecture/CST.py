...
class XBOXPLUS(AsymmetricTransform):
    ...
    def set_norms(self, x):
        self.x_norms = x.norm(p=2, dim=-1, keepdim=True)  # 在最后一个维度上计算二范数（每个元素的平方和开根号）（bs,x_seqlen,1）
        self.MX = torch.amax(self.x_norms, dim=-2, keepdim=True)  # (bs,1,1)
    ...
    def X(self, x):
        ...
        return torch.cat((x, ext, zero), -1)  # (bs,x_seqlen,dim+1+1)


def lsh_clustering(x, n_rounds, r=1):
    salsh = SALSH(n_rounds=n_rounds, dim=x.shape[-1], r=r, device=x.device)  # 计算Hash
    x_hashed = salsh(x).reshape((n_rounds,) + x.shape[:-1])
    return x_hashed.argsort(dim=-1)  # 返回排序后的下标
...
class SALSH(LSH):
    ...
    def __call__(self, vecs):
        projection = vecs @ self.alpha  # (bs,x_seqlen,n_rounds)
        projection_shift = projection + self.beta
        projection_rescale = projection_shift / self.r
        return projection_rescale.permute(2, 0, 1)  # (n_rounds,bs,x_seqlen)
...
def batch_scatter(output, src, dim, index):
    ...
    :return: output: [b,n,c]
    """
    b, k, c = src.shape  # 对应x_select
    index = index[:, :, None].expand(-1, -1, c)  # (b, n, c)
    output, src, index = map(lambda t: rearrange(t, 'b k c -> (b c) k'), (output, src, index))
    output.scatter_(dim, index, src)  # 根据Index将src中的块映射到output上，output是原先所有的未筛选的块，这样把学过注意力的块进行替换
...
def batch_gather(x, index, dim):
    ...
    """
    b, n, c = x.shape
    index = index[:, :, None].expand(-1, -1, c)  # （b,n//2,c）
    x, index = map(lambda t: rearrange(t, 'b n c -> (b c) n'), (x, index))
    output = torch.gather(x, dim, index)  # 根据Index对x进行块的挑选
    output = rearrange(output, '(b c) n -> b n c', b=b)  # 对挑选出来的块进行reshape
    return output
...
class SAH_MSA(nn.Module):
    def __init__(self, heads=4, n_rounds=2, channels=64, patch_size=(8, 8),
                 r=1):
    ...
    def forward(self, input):
        ...
        B, N, C_inp = input.shape  # [b*nh*nw//2, hh*ww，c] 所以N=16*16，B=b*总块数的一半(即挑选出来的块数)
        query = self.to_q(input)#(B,N,C_inp*3)
        key = self.to_k(input)
        value = self.to_v(input)
        input_hash = input.view(B, N, self.heads, C_inp // self.heads)  # 沿着通道方向分头
        x_hash = rearrange(input_hash, 'b t h e -> (b h) t e')
        bs, x_seqlen, dim = x_hash.shape  # x_seqlen=N=hh*ww=16*16
        with torch.no_grad():
        ...
        C = query.shape[-1]  # inner_dim=3 * channels
        query = query.view(B, N, self.heads, C // self.heads)#分头
        ...
        bs, q_seqlen, dim = query.shape#q_seqlen=N=hh*ww
        bs, k_seqlen, dim = key.shape
        v_dim = value.shape[-1]

        x_rev_positions = torch.argsort(x_positions, dim=-1)
        x_offset = torch.arange(bs, device=query.device).unsqueeze(-1) * x_seqlen# (bs, 1)
...
class SAHAB(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        w_size = self.patch_size  # (8,8)

        # Split into large patches b,h,w,c->b,N,s
        x = rearrange(x, 'b (nh hh) (nw ww) c-> b (nh nw) (hh ww c)', hh=w_size[0] * 2,
                      ww=w_size[1] * 2)  # 分块，每块（16,16,c）,一共nh*nw块
        mask = rearrange(mask, 'b (nh hh) (nw ww) -> b (nh nw) (hh ww)', hh=w_size[0] * 2,
                         ww=w_size[1] * 2)  # 分块，每块（16,16）,一共nh*nw块
        N = x.shape[1]  # 块数N=nh*nw

        mask = torch.mean(mask, dim=2, keepdim=False)  # [b,nh*nw] 求每块的平均值，对应论文中使用average pooling进行下采样
        if self.sparse:
            mask_select = mask.topk(mask.shape[1] // 2, dim=1)[
                1]  # [b,nh*nw//2]选前一半，返回的是top k 的 value和index，这里[1]表示取index
            x_select = batch_gather(x, mask_select, 1)  # [b,nh*nw//2,hh*ww*c] 选出前一半对应的x块
            x_select = x_select.reshape(b * N // 2, -1, c)  # [b*nh*nw//2, hh*ww，c]
            x_select = self.attn(x_select) + x_select
            x_select = x_select.view(b, N // 2, -1)  # [b，nh*nw//2, hh*ww*c]
...
class Sparsity_Estimator(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sparse:
            error_map = out[:, -1:, :, :]  # 最后一个通道是error_map
