...
class HyPaNet(nn.Module):
    ...
    def forward(self, x):
        x = self.dab(self.relu(self.fution(x)))
        x = self.avg_pool(x)
        x = self.mlp(x) + 1e-6
...
from .DualMambaNet import DualMambaNet
class DHM(nn.Module):
    def __init__(self, num_iterations=1, patch_size=256):
        ...
        for _ in range(num_iterations):
            self.denoisers.append(
                DualMambaNet(in_dim=29, out_dim=28, dim=28, num_blocks=[1,1,1], patch_size=patch_size),
            )
    ...
    def forward(self, y, Phi=None, Phi_s=None):
        """
        :param y: [b,256,310]
        :param Phi: [b,28,256,310]
