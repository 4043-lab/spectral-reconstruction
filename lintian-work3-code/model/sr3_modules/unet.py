...
class UNet(nn.Module):
    def __init__(
            ...
            for i in range(0, res_blocks):
                if i == res_blocks-1:
                    ...
                    downs.append(ResnetBlocWithAttn(
                        pre_channel, channel_mult, noise_level_emb_dim=noise_level_channel, norm_groups=norm_groups, dropout=dropout,
                        with_attn=False))
                # downs.append(ResnetBlocWithAttn(
        ...
        self.mid = nn.ModuleList([
            ...
            ResnetBlocWithAttn(pre_channel, pre_channel, noise_level_emb_dim=noise_level_channel, norm_groups=norm_groups,
                               dropout=dropout, with_attn=False),
            TrainableResnetBlocWithAttnMid(pre_channel, pre_channel, noise_level_emb_dim=noise_level_channel, norm_groups=norm_groups,
                                           dropout=dropout, with_attn=True)
        ])
            ...
            for i in range(0, res_blocks+1):
                ...
                if i == res_blocks:
                    ups.append(TrainableResnetBlocWithAttnDec(
                        pre_channel + feat_channels.pop(), channel_mult, noise_level_emb_dim=noise_level_channel, dropout=dropout,
                        norm_groups=norm_groups, with_attn=False))
                else:
                    ups.append(ResnetBlocWithAttn(
                        pre_channel+feat_channels.pop(), channel_mult, noise_level_emb_dim=noise_level_channel, dropout=dropout, norm_groups=norm_groups, with_attn=False))
        ...
        self.mask_tail = FCN(out_channel*2)
...
class FCN(nn.Module):
    def __init__(self, inchanels=48):
        super(FCN, self).__init__()
        self.fcn = nn.Sequential(
            # nn.Conv2d(4, 32, 3, padding=1),
            # nn.ReLU(inplace=True),
            # nn.Conv2d(inchanels, inchanels, 3, padding=1),
            # nn.ReLU(inplace=True),
            nn.Conv2d(inchanels, inchanels, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(inchanels, inchanels, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(inchanels, 1, 1, 1, 0),
            nn.Sigmoid()
        )
