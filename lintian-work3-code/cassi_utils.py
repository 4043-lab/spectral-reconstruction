...
def generate_masks(mask_path, batch_size, inchannels=28, patch_size = 256):
    ...
    x_index = 0
    y_index = 0
    mask_crop = mask3d[:, x_index:x_index + patch_size, y_index:y_index + patch_size]
    mask3d_batch = mask_crop.expand([batch_size, nC, patch_size, patch_size]).cuda().float()
    return mask3d_batch
...
def prepare_data_KAIST(path, file_num):
    ...
    file_list.sort()
    print('training scenes:', len(file_list))
...
def shuffle_crop(train_data, batch_size, crop_size=256, channels=28, argument=True):
    if argument:
        ...
        # x_index = np.random.randint(0, h - crop_size)
        # y_index = np.random.randint(0, w - crop_size)
        # mask_crop = mask[:, :, x_index:x_index + crop_size, y_index:y_index + crop_size]
        return gt_batch
    else:
...
def shuffle_crop_real(train_data, batch_size, crop_size=256, channels=28, argument=True):
    if argument:
        gt_batch = []
        # The first half data use the original data.
        index = np.random.choice(range(len(train_data)), batch_size)
        processed_data = np.zeros((batch_size, crop_size, crop_size, channels), dtype=np.float32)
        for i in range(batch_size):
            img = train_data[index[i]]
            h, w, c = img.shape
            assert c == channels
            x_index = np.random.randint(0, h - crop_size)
            y_index = np.random.randint(0, w - crop_size)
            processed_data[i, :, :, :] = img[x_index:x_index + crop_size, y_index:y_index + crop_size, :] #b, h, w, c
        for i in range(processed_data.shape[0]):
            gt_batch.append(arguement_1_np(processed_data[i]))
        gt_batch = np.stack(gt_batch, axis=0)
        gt_batch = np.transpose(gt_batch, (0, 3, 1, 2))#b, c, h ,w
        return gt_batch
        ...
        # for i in range(processed_data.shape[0]):
        #     gt_batch.append(arguement_1(processed_data[i]))
        # gt_batch = torch.stack(gt_batch, dim=0).cpu().
        # return gt_batch
    else:
        index = np.random.choice(range(len(train_data)), batch_size)
        processed_data = np.zeros((batch_size, crop_size, crop_size, 28), dtype=np.float32)
...
def gen_meas_KAIST(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    nC = data_batch.shape[1]
    mask3d_batch = mask3d_batch.cpu().numpy()
    temp = shift_np(mask3d_batch * data_batch, 2)
    meas = np.sum(temp, 1)
    QE, bit = 0.4, 2048
    meas = np.random.binomial((meas * bit / QE).astype(int), QE)
    meas = torch.from_numpy(np.float32(meas) / np.float32(bit)).cuda().float()
    if Y2H:
        meas_norm = meas / nC * 2
...
def gen_meas_torch_real(data_batch, mask3d_batch, Y2H=True, mul_mask=False, in_channels=28):
    meas = data_batch
    if Y2H:
        H = shift_back(meas, step=2, in_channels=in_channels)
        if mul_mask:
            HM = torch.mul(H, mask3d_batch)
            return HM, meas
        return H, meas
...
def shift_np(inputs, step=2): # input [bs,28,256,256]  output [bs, 28, 256, 310]
    [bs, nC, row, col] = inputs.shape
...
def shift_back_np(inputs, step=2, in_channels=28):  # input [bs,256,310]  output [bs, 28, 256, 256]
    [bs, row, col] = inputs.shape
    # print(inputs.shape)
    nC = in_channels
    output = np.zeros((bs, nC, row, col - (nC - 1) * step))
...
def init_mask(mask_path, mask_type, batch_size, in_channels=28, patch_size = 256):
    input_mask = None
    mask3d_batch = generate_masks(mask_path, batch_size, in_channels, patch_size)
    if mask_type == 'Phi':
...
def init_meas(gt, mask, input_setting, in_channels=28):
    ...
    meas = None
    if input_setting == 'H':
        input_meas, meas = gen_meas_torch(gt, mask, Y2H=True, mul_mask=False, in_channels=in_channels)
    elif input_setting == 'HM':
        input_meas, meas = gen_meas_torch(gt, mask, Y2H=True, mul_mask=True, in_channels=in_channels)
    elif input_setting == 'Y':
        input_meas, meas = gen_meas_torch(gt, mask, Y2H=False, mul_mask=False, in_channels=in_channels)
    return input_meas, meas

def init_meas_real_test(gt, mask, input_setting, in_channels=28):
    input_meas = None
    meas = None
    if input_setting == 'H':
        input_meas, meas = gen_meas_torch_real(gt, mask, Y2H=True, mul_mask=False, in_channels=in_channels)
    elif input_setting == 'HM':
        input_meas, meas = gen_meas_torch_real(gt, mask, Y2H=True, mul_mask=True, in_channels=in_channels)
    elif input_setting == 'Y':
        input_meas, meas = gen_meas_torch_real(gt, mask, Y2H=False, mul_mask=False, in_channels=in_channels)
    return input_meas, meas

def init_meas_KAIST(gt, mask, input_setting, in_channels=28):
    input_meas = None
    meas = None
    if input_setting == 'H':
        input_meas, meas = gen_meas_KAIST(gt, mask, Y2H=True, mul_mask=False)
    elif input_setting == 'HM':
        input_meas, meas = gen_meas_KAIST(gt, mask, Y2H=True, mul_mask=True)
    elif input_setting == 'Y':
        input_meas, meas = gen_meas_KAIST(gt, mask, Y2H=False, mul_mask=False)
    return input_meas, meas

def checkpoint(model, epoch, model_path, logger, psnr=0.0, ssim=0.0):
...
def my_summary(test_model, H = 256, W = 256, C = 28, N = 1):
    ...
    macs, params = get_model_complexity_info(model, (256, 310), as_strings=True, print_per_layer_stat=False)
