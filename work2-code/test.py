...
# model
if opt.method=='hdnet':
    model, FDL_loss = model_generator(opt, opt.method, opt.pretrained_model_path)
...
def test():
    ...
    input_meas = init_meas(test_gt, mask3d_batch_test, opt.input_setting, opt.in_channels)
    model.eval()
    begin = time.time()
    with torch.no_grad():
    ...
    print('===> testing psnr = {:.2f}, ssim = {:.3f}, sam = {:.3f}, rmse = {:.3f}, ergas = {:.3f}, scc = {:.3f}, time: {:.2f}'.format(psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean, (end - begin)))
    ...
    if opt.method.find('my_model') >= 0:
        ...
        meas = np.transpose(phi[2].cpu().numpy(), (0, 2, 3, 1)).astype(np.float32)
        phi = np.transpose(phi[1].cpu().numpy(), (0, 2, 3, 1)).astype(np.float32)

        return pred, truth, psnr_list, ssim_list, sam_list, rmse_list, ergas_list, scc_list, psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean, phi, mask, meas
...
if __name__ == '__main__':
    ...
    torch.backends.cudnn.benchmark = True
    if opt.method.find('my_model') >= 0:
        (pred, truth, psnr_all, ssim_all, sam_all, rmse_all, ergas_all, scc_all, psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean, phi, mask, meas) = test()
        name = result_path + '/' + 'Test_psnr:{:.2f}_ssim:{:.3f}_sam:{:.3f}_rmse:{:.3f}_ergas:{:.3f}_scc:{:.3f}'.format(psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean) + '.mat'
        scio.savemat(name, {'pred': pred, 'psnr_list': psnr_all, 'ssim_list': ssim_all, 'sam_list': sam_all, 'rmse_list': rmse_all, 'ergas_list': ergas_all, 'scc_list': scc_all, 'phi':phi, 'mask':mask, 'meas':meas})
    else:
        (pred, truth, psnr_all, ssim_all, sam_all, rmse_all, ergas_all, scc_all, psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean) = test()
        name = result_path + '/' + 'Test_psnr:{:.2f}_ssim:{:.3f}_sam:{:.3f}_rmse:{:.3f}_ergas:{:.3f}_scc:{:.3f}'.format(psnr_mean, ssim_mean, sam_mean, rmse_mean, ergas_mean, scc_mean) + '.mat'
        scio.savemat(name, {'truth': truth, 'pred': pred, 'psnr_list': psnr_all, 'ssim_list': ssim_all, 'sam_list': sam_all, 'rmse_list': rmse_all, 'ergas_list': ergas_all, 'scc_list': scc_all})
    print("complete!")
